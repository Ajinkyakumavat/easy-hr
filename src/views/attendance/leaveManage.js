...
const LeaveManage = () => {
    ...
    const [showIndex, setshowIndex] = React.useState(0);
        const { Leave, setLeave } = useContext(AuthContextProvider)
    ...
    React.useEffect(() => {
        ...
        async function getAtt() {
            const x = await axios.get(` http://52.86.15.74:4000/api/v1/employee/attendance/mylist/${date?.getFullYear()}?page=${page}`, {
    ...
    React.useEffect(() => {
         if (Leave === true) {
            toast.success('Attandance updated successfully');
            setLeave(false)
        }
