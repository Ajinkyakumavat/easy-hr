...
import WeeklyOff from 'ui-component/weeklyoffreportbtn';
...
function reportDownload() {
    ...
    pdf.html(pdfTable, { html2canvas: { scale: 0.98 } }).then(() => {
        pdf.save('weeklyoff_report.pdf');
    });
}

const WeeklyOffReport = () => {
    ...
    const getAttendence = (employee, currdate) => {
        let x =
            employeeAttendance.findIndex((item) => item?.employee == employee) > -1 &&
            employeeAttendance?.filter((item) => item?.employee == employee)[0]?.employeeAttendance?.filter((item) => item.date == currdate)
                .length > 0
                ? employeeAttendance
                      ?.filter((item) => item?.employee == employee)[0]
                      ?.employeeAttendance?.filter((item) => item.date == currdate)[0]
                : {};
        //console.log(x);
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <TableCell align="center">
                                                    <WeeklyOff row={row} x={x} date={date} getAtt={getAttendence} />
                                                </TableCell>
                                            </StyledTableRow>
                                        );
                                    })}
                ...
                <table id="capture" style={{ textAlign: 'center', width: '100%', borderCollapse: 'collapse', margin: '10px' }} width="100%">
                    <thead>
                        ...
                        <tr>
                            ...
                            <th style={{ border: '1px solid black', borderCollapse: 'collapse', padding: '5px' }}>Phone</th>
...
export default WeeklyOffReport;
